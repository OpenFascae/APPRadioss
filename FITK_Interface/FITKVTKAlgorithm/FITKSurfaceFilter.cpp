...
{
    const int qsize = sizeof(FITKFastGeomQuad);
    const int sizeId = sizeof(vtkIdType);
    // If necessary, we create padding after FITKFastGeomQuad such that
...
class FITKSurfaceFilter::vtkEdgeInterpolationMap
{
public:
...
vtkObjectFactoryNewMacro(FITKSurfaceFilter);

int FITKSurfaceFilter::getOriginCellIdBySurfaceCellId(int surfaceCellId)
{
    if (surfaceCellId < 0 || _originAndSurfaceCellMap.find(surfaceCellId) == _originAndSurfaceCellMap.end())
    {
        return -1;
    }

    return _originAndSurfaceCellMap[surfaceCellId].first;
}

int FITKSurfaceFilter::getOriginPointIdBySurfacePointId(int surfacePointId)
{
    if (_pointMap.size() == 0 || _pointMap.find(surfacePointId) == _pointMap.end())
    {
        return -1;
    }

    return _pointMap[surfacePointId];
}

int FITKSurfaceFilter::getSurfacePointIdByOriginPointId(int orgPointId)
{
    if (_pointMapOS.size() == 0 || _pointMapOS.find(orgPointId) == _pointMapOS.end())
    {
        return -1;
    }


    return _pointMapOS[orgPointId];
}

void FITKSurfaceFilter::getSurfaceCellInfo(int surfaceCellId, int& cellId, int& surfaceIndex)
{
    cellId = -1;
    surfaceIndex = -1;
    ...
    std::pair<int, int> & pair = _originAndSurfaceCellMap[surfaceCellId];
    cellId = pair.first;
    surfaceIndex = pair.second;
}

int FITKSurfaceFilter::getSurfaceCellIdByOriginCellId(int originCellId, int surfaceIndex)
{
    if (originCellId < 0) {
        return -1;
    }

    for (const auto& pair : _originAndSurfaceCellMap) {
        //获取原始单元ID与原始单元索引对应关系
        std::pair<int, int> info = pair.second;
        if (info.first != originCellId) {
            continue;
        }
        //判断是否要根据面索引查询
        if (surfaceIndex == -1) {
...
//----------------------------------------------------------------------------
FITKSurfaceFilter::FITKSurfaceFilter()
{
...
//----------------------------------------------------------------------------
int FITKSurfaceFilter::RequestData(
    ...
    vtkInformationVector *outputVector)
{
    ...
    _pointMap.clear();
    _pointMapOS.clear();
    _originAndSurfaceCellMap.clear();
    ...
    case VTK_POLY_DATA:
    {
        ...
        this->OriginalCellDims = vtkIntArray::New();
        this->OriginalCellDims->SetName(this->GetOriginalCellDimsName());
        this->OriginalCellDims->SetNumberOfComponents(1);
        output->GetCellData()->AddArray(this->OriginalCellDims);
        vtkIdType numTup = output->GetNumberOfCells();
        this->OriginalCellDims->SetNumberOfValues(numTup);
        for (vtkIdType cId = 0; cId < numTup; cId++)
        {
            this->OriginalCellDims->SetValue(cId, 2);
        }
        this->OriginalCellDims->Delete();
        this->OriginalCellDims = nullptr;
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::EstimateStructuredDataArraySizes(
...
//----------------------------------------------------------------------------
int FITKSurfaceFilter::UniformGridExecute(
...
int FITKSurfaceFilter::StructuredExecute(vtkDataSet *input,
    vtkPolyData *output,
    vtkIdType *ext,
    vtkIdType *wholeExt)
{
    ...
    if (rgrid || sgrid)
    {
        ...
        if (dimension == 1)
        {
            ...
            if (rgrid)
            {
                vtkSmartPointer<vtkRectilinearGridGeometryFilter> filter = vtkSmartPointer<vtkRectilinearGridGeometryFilter>::New();
    ...
    if (this->OriginalCellDims != nullptr)
    {
        this->OriginalCellDims->Delete();
        this->OriginalCellDims = nullptr;
    }
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::ExecuteFaceStrips(vtkDataSet *input,
    vtkPolyData *output,
        ...
        if (rotatedFlag)
        {
            for (ib = ext[bA2]; ib <= ext[bA2 + 1]; ++ib)
            {
                ...
                if (this->PassThroughCellIds && ib != ext[bA2])
                {
                    ...
                    this->RecordOrigCellId(outCellId++, inCellId, 2);
                    this->RecordOrigCellId(outCellId++, inCellId, 2);
                }
            }
        }
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::ExecuteFaceQuads(vtkDataSet *input,
    vtkPolyData *output,
    int maxFlag, vtkIdType *ext,
    int aAxis, int bAxis, int cAxis,
    vtkIdType *wholeExt,
    ...
    for (ic = ext[cA2]; ic < ext[cA2 + 1]; ++ic)
    {
        for (ib = ext[bA2]; ib < ext[bA2 + 1]; ++ib)
        {
            ...
            if (checkVisibility && grid->IsCellVisible(inId))
            {
                ...
                this->RecordOrigCellId(outId, inId, 2);
            }
        }
    }
...
#if VTK_MAJOR_VERSION >= 9
//----------------------------------------------------------------------------
int FITKSurfaceFilter::StructuredWithBlankingExecute(vtkStructuredGrid *input,
    vtkPolyData *output)
{
    vtkIdType newCellId;
    ...
    this->OriginalCellDims->SetName(this->GetOriginalCellDimsName());
    this->OriginalCellDims->SetNumberOfComponents(1);
    outputCD->AddArray(this->OriginalCellDims);
        ...
        for (int j = 0; j < dims[1]; ++j)
        {
            for (int i = 0; i < dims[0]; ++i)
            {
                ...
                case 2:
                    ...
                    this->RecordOrigCellId(newCellId, cellId, cellDim);
                    break;
                case 3:
                    int even[3] = { i,j,k };
...
//----------------------------------------------------------------------------
int FITKSurfaceFilter::DataSetExecute(vtkDataSet *input,
    vtkPolyData *output)
{
...
//----------------------------------------------------------------------------
int FITKSurfaceFilter::RequestUpdateExtent(
    vtkInformation *vtkNotUsed(request),
    vtkInformationVector **inputVector,
    vtkInformationVector *outputVector)
{
...
//----------------------------------------------------------------------------
int FITKSurfaceFilter::FillInputPortInformation(int, vtkInformation *info)
{
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
int FITKSurfaceFilter::UnstructuredGridExecute(vtkDataSet *dataSetInput,
    vtkPolyData *output)
{
    ...
    if (handleSubdivision)
    {
        ...
        vtkSmartPointer<vtkUnstructuredGridGeometryFilter> uggf = vtkSmartPointer<vtkUnstructuredGridGeometryFilter>::New();
        vtkSmartPointer<vtkUnstructuredGrid> clone = vtkSmartPointer<vtkUnstructuredGrid>::New();
    ...
    int numFacePts, numCellPts, cellDim;
        ...
        cellIter->GoToNextCell())
    {
        ...
        if (cellType == VTK_VERTEX || cellType == VTK_POLY_VERTEX)
        {
            ...
            this->RecordOrigCellId(this->NumberOfNewCells, cellId, 0);
        ...
        cellIter->GoToNextCell())
    {
        ...
        case VTK_POLY_LINE:
            ...
            this->RecordOrigCellId(this->NumberOfNewCells, cellId, 1);
            outputCD->CopyData(cd, cellId, this->NumberOfNewCells++);
            break;
            //C3D8线性六面体
        case VTK_HEXAHEDRON:
            ...
            this->InsertQuadInHash(ids[0], ids[1], ids[5], ids[4], cellId, 3);
            this->InsertQuadInHash(ids[0], ids[3], ids[2], ids[1], cellId, 3);
            this->InsertQuadInHash(ids[0], ids[4], ids[7], ids[3], cellId, 3);
            this->InsertQuadInHash(ids[1], ids[2], ids[6], ids[5], cellId, 3);
            this->InsertQuadInHash(ids[2], ids[3], ids[7], ids[6], cellId, 3);
            this->InsertQuadInHash(ids[4], ids[5], ids[6], ids[7], cellId, 3);
            break;
        ...
        case VTK_VOXEL:
            ...
            ids = pointIdList->GetPointer(0);
            this->InsertQuadInHash(ids[0], ids[1], ids[5], ids[4], cellId, 3);
            this->InsertQuadInHash(ids[0], ids[2], ids[3], ids[1], cellId, 3);
            this->InsertQuadInHash(ids[0], ids[4], ids[6], ids[2], cellId, 3);
            this->InsertQuadInHash(ids[1], ids[3], ids[7], ids[5], cellId, 3);
            this->InsertQuadInHash(ids[2], ids[6], ids[7], ids[3], cellId, 3);
            this->InsertQuadInHash(ids[4], ids[5], ids[7], ids[6], cellId, 3);
            break;
            //C3D4线性四面体
        case VTK_TETRA:
            pointIdList = cellIter->GetPointIds();
            ids = pointIdList->GetPointer(0);
            this->InsertTriInHash(ids[0], ids[1], ids[3], cellId, 3, 2);
            this->InsertTriInHash(ids[0], ids[2], ids[1], cellId, 3, 3);
            this->InsertTriInHash(ids[0], ids[3], ids[2], cellId, 3, 1);
            this->InsertTriInHash(ids[1], ids[2], ids[3], cellId, 3, 0);
            break;

        case VTK_PENTAGONAL_PRISM:
            pointIdList = cellIter->GetPointIds();
            ids = pointIdList->GetPointer(0);
            this->InsertQuadInHash(ids[0], ids[1], ids[6], ids[5], cellId, 3);
            this->InsertQuadInHash(ids[1], ids[2], ids[7], ids[6], cellId, 3);
            this->InsertQuadInHash(ids[2], ids[3], ids[8], ids[7], cellId, 3);
            this->InsertQuadInHash(ids[3], ids[4], ids[9], ids[8], cellId, 3);
            this->InsertQuadInHash(ids[4], ids[0], ids[5], ids[9], cellId, 3);
            this->InsertPolygonInHash(ids, 5, cellId, 3);
            this->InsertPolygonInHash(&ids[5], 5, cellId, 3);
            break;

        case VTK_HEXAGONAL_PRISM:
            pointIdList = cellIter->GetPointIds();
            ids = pointIdList->GetPointer(0);
            this->InsertQuadInHash(ids[0], ids[1], ids[7], ids[6], cellId, 3);
            this->InsertQuadInHash(ids[1], ids[2], ids[8], ids[7], cellId, 3);
            this->InsertQuadInHash(ids[2], ids[3], ids[9], ids[8], cellId, 3);
            this->InsertQuadInHash(ids[3], ids[4], ids[10], ids[9], cellId, 3);
            this->InsertQuadInHash(ids[4], ids[5], ids[11], ids[10], cellId, 3);
            this->InsertQuadInHash(ids[5], ids[0], ids[6], ids[11], cellId, 3);
            this->InsertPolygonInHash(ids, 6, cellId, 3);
            this->InsertPolygonInHash(&ids[6], 6, cellId, 3);
            break;

        case VTK_PYRAMID:
            pointIdList = cellIter->GetPointIds();
            ids = pointIdList->GetPointer(0);
            this->InsertQuadInHash(ids[3], ids[2], ids[1], ids[0], cellId, 3);
            this->InsertTriInHash(ids[0], ids[1], ids[4], cellId, 3);
            this->InsertTriInHash(ids[1], ids[2], ids[4], cellId, 3);
            this->InsertTriInHash(ids[2], ids[3], ids[4], cellId, 3);
            this->InsertTriInHash(ids[3], ids[0], ids[4], cellId, 3);
            break;
            //C3D6楔形
        case VTK_WEDGE:
            pointIdList = cellIter->GetPointIds();
            ids = pointIdList->GetPointer(0);
            this->InsertQuadInHash(ids[0], ids[2], ids[5], ids[3], cellId, 3);
            this->InsertQuadInHash(ids[1], ids[0], ids[3], ids[4], cellId, 3);
            this->InsertQuadInHash(ids[2], ids[1], ids[4], ids[5], cellId, 3);
            this->InsertTriInHash(ids[0], ids[1], ids[2], cellId, 3);
            this->InsertTriInHash(ids[3], ids[5], ids[4], cellId, 3);
            break;
                            ...
                            if (this->NonlinearSubdivisionLevel >= 1)
                            {
                                ...
                                for (i = 0; i < pts->GetNumberOfIds(); i += 3)
                                {
                                    this->InsertTriInHash(pts->GetId(i), pts->GetId(i + 1),
                                        pts->GetId(i + 2), cellId, 3);
                                }
                            }
                            else
                            {
                                // Modified by ChengHaotian 2025/07/24
                                switch (face->GetCellType())
                                {
                                case VTK_QUADRATIC_TRIANGLE:
                                {
                                    vtkIdType* qcids = face->PointIds->GetPointer(0);
                                    vtkIdType ids[6]{ qcids[0], qcids[3], qcids[1], qcids[4], qcids[2], qcids[5] };
                                    this->InsertPolygonInHash(ids, 6, cellId, 3);
                                    break;
                                }
#if VTK_MAJOR_VERSION >= 9
                                ...
                                {
                                    vtkIdType* qcids = face->PointIds->GetPointer(0);
                                    vtkIdType ids[8]{ qcids[0], qcids[4], qcids[1], qcids[5], qcids[2], qcids[6], qcids[3], qcids[7] };
                                    this->InsertPolygonInHash(ids, 8, cellId, 3);
                                    break;
                                }
#if VTK_MAJOR_VERSION >= 9
                                case VTK_LAGRANGE_QUADRILATERAL:
        ...
        cellIter->GoToNextCell())
    {
        ...
        cellDim = cellIter->GetCellDimension();
        ...
        if (this->NonlinearSubdivisionLevel < 1)
        {
            // Modified by ChengHaotian 2025/07/25
            switch (cellType)
            {
            case VTK_QUADRATIC_TRIANGLE:
                break;
        ...
        { // Do we really want to insert the 2D cells into a hash?
            ...
            this->RecordOrigCellId(this->NumberOfNewCells, cellId, cellDim);
            outputCD->CopyData(cd, cellId, this->NumberOfNewCells++);
        }
        ...
        // Added by ChengHaotian 2025/07/25
            ...
            || cellType == VTK_BIQUADRATIC_TRIANGLE)
        {
            ...
            vtkIdType idsPoly[6]{ ids[0], ids[3], ids[1], ids[4], ids[2], ids[5] };
            for (i = 0; i < 6; i++)
            {
                inPtId = idsPoly[i];
                outPtId = this->GetOutputPointId(inPtId, input, newPts, outputPD);
            ...
            || cellType == VTK_QUADRATIC_LINEAR_QUAD)
        {
            ...
            vtkIdType idsPoly[8]{ ids[0], ids[4], ids[1], ids[5], ids[2], ids[6], ids[3], ids[7] };
            for (i = 0; i < 8; i++)
            {
                inPtId = idsPoly[i];
                outPtId = this->GetOutputPointId(inPtId, input, newPts, outputPD);
...
//#endif
//            )
         else if (cellType == VTK_QUADRATIC_POLYGON
#if VTK_MAJOR_VERSION >= 9
            || cellType == VTK_LAGRANGE_TRIANGLE
            || cellType == VTK_LAGRANGE_QUADRILATERAL
#endif
    ...
    while ((q = this->GetNextVisibleQuadFromHash()))
    {
        ...
        vtkUnstructuredGrid* inputGrid = dynamic_cast<vtkUnstructuredGrid*>(input);

        // Added by ChengHaotian
        //@{
        if (this->PassThroughPointIds && this->PassThroughCellIds)
        {
            vtkCell* orgCell = inputGrid->GetCell(q->SourceId);

            int cellFaceId = -1;
            int nOrgCellFace = orgCell->GetNumberOfFaces();
            for (int i = 0; i < nOrgCellFace; i++)
            {
                vtkCell* face = orgCell->GetFace(i);
                vtkIdList* facePointIds = face->GetPointIds();

                bool isTheFace = q->numPts != 0;

                for (int j = 0; j < q->numPts; j++)
                {
                    int newCellNewPtId = q->ptArray[j];
                    int newCellOrgPtId = this->OriginalPointIds->GetValue(newCellNewPtId);

                    if (facePointIds->IsId(newCellOrgPtId) == -1)
                    {
                        isTheFace = false;
                        break;
                    }
            ...
            this->RecordOrigCellId(this->NumberOfNewCells, q, cellFaceId);
        }
        //@}
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::InitializeQuadHash(vtkIdType numPoints)
{
    vtkIdType i;
    ...
    this->QuadHash = new FITKFastGeomQuad*[numPoints];
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::DeleteQuadHash()
{
    vtkIdType i;
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::InsertQuadInHash(vtkIdType a, vtkIdType b,
    vtkIdType c, vtkIdType d,
    vtkIdType sourceId, int cellDim)
{
    vtkIdType tmp;
    FITKFastGeomQuad *quad, **end;
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::InsertTriInHash(vtkIdType a, vtkIdType b,
    vtkIdType c, vtkIdType sourceId, int cellDim,
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::InsertPolygonInHash(vtkIdType* ids,
    int numPts, vtkIdType sourceId, int cellDim)
{
    FITKFastGeomQuad *quad, **end;
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::InitFastGeomQuadAllocation(vtkIdType numberOfCells)
{
    int idx;
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::DeleteAllFastGeomQuads()
{
...
//----------------------------------------------------------------------------
FITKFastGeomQuad* FITKSurfaceFilter::NewFastGeomQuad(int numPts)
{
    ...
    FITKFastGeomQuad* q = reinterpret_cast<FITKFastGeomQuad*>
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::InitQuadHashTraversal()
{
...
//----------------------------------------------------------------------------
FITKFastGeomQuad *FITKSurfaceFilter::GetNextVisibleQuadFromHash()
{
    FITKFastGeomQuad *quad;
...
//----------------------------------------------------------------------------
vtkIdType FITKSurfaceFilter::GetOutputPointId(vtkIdType inPtId,
    vtkDataSet *input,
    vtkPoints *outPts,
    vtkPointData *outPD)
{
...
//-----------------------------------------------------------------------------
vtkIdType FITKSurfaceFilter::GetInterpolatedPointId(vtkIdType edgePtA,
    vtkIdType edgePtB,
    vtkDataSet *input,
    vtkCell *cell,
    double pcoords[3],
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::RecordOrigCellId(vtkIdType destIndex,
    vtkIdType originalId, vtkIdType cellDim,
    vtkIdType oriSurIndex)
{
    if (this->OriginalCellIds != nullptr)
    {
    ...
    if (this->OriginalCellDims != nullptr)
    {
        this->OriginalCellDims->InsertValue(destIndex, cellDim);
    }
    ...
    _originAndSurfaceCellMap[destIndex] = std::pair<int, int>(originalId, oriSurIndex);
}
    ...
    vtkIdType        oriSurIndex)
{
    ...
    if (this->OriginalCellDims != nullptr)
    {
        this->OriginalCellDims->InsertValue(destIndex, quad->CellDim);
    }
...
//----------------------------------------------------------------------------
void FITKSurfaceFilter::RecordOrigPointId(vtkIdType destIndex,
    vtkIdType originalId)
{
    if (this->OriginalPointIds != nullptr)
    {
        ...
        _pointMap[destIndex] = originalId;
        _pointMapOS[originalId] = destIndex;
    }
}
