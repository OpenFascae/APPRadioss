...
FITKPolygonSource::FITKPolygonSource()
{
    ...
    this->Normal[1] = 0.0;
    this->Normal[2] = 1.0;
    this->PlaneViewUp[0] = 0.0;
    this->PlaneViewUp[1] = 1.0;
    this->PlaneViewUp[2] = 0.0;
    this->Radius = 0.5;
    ...
    vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
    ...
    axis[0] = this->PlaneViewUp[0];
    axis[1] = this->PlaneViewUp[1];
    axis[2] = this->PlaneViewUp[2];
    ...
    else if (this->NumberOfSides == 3)
    {
        for (j = 0; j < 3; j++)
        {
            for (i = 0; i < 3; i++)
            {
                r[i] = px[i] * cos((double)j * theta - theta * 0.25) + py[i] * sin((double)j * theta - theta * 0.25);
                x[i] = this->Center[i] + this->Radius * r[i];
            }
