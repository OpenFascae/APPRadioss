...
#include "GUIFileDirectoryDialog.h"
#include "ui_GUIFileDirectoryDialog.h"
...
namespace GUI
{
    const QString GUIFileDirectoryDialog::fatherNodeText = "FileName"; //根节点名称
    const QString GUIFileDirectoryDialog::splitExt = "@";    //用于文件头类型分割符，如12@dat

    GUIFileDirectoryDialog::GUIFileDirectoryDialog(Core::FITKActionOperator* oper, QWidget *parent)
        : GUIDialogBase(parent), _oper(oper)
    {
        _ui = new Ui::GUIFileDirectoryDialog;
        _ui->setupUi(this);
        ...
        QStringList names;
        names << GUIFileDirectoryDialog::fatherNodeText;
    ...
    GUIFileDirectoryDialog::~GUIFileDirectoryDialog()
    {
        _fileTree->clear();
        _pathBackVec.clear();
        _pathFrontVec.clear();
    ...
    void GUIFileDirectoryDialog::initUi()
    {
        //ini combox
    ...
    void GUIFileDirectoryDialog::BuildFilePathTree()
    {
        _fileTree->clear();
        QStringList names;
        names << GUIFileDirectoryDialog::fatherNodeText;
    ...
    void GUIFileDirectoryDialog::initWin(QString dirStr)
    {
    ...
    QString GUIFileDirectoryDialog::appendSlash(QString curPath)
    {
    ...
    void GUIFileDirectoryDialog::openFilePath(bool recordHistory)
    {
    ...
    QString GUIFileDirectoryDialog::GetFileExt(QString fileName)
    {
        QFileInfo fileInfo;
    ...
    QStringList GUIFileDirectoryDialog::GetFilterFromString(const QString& filter)
    {
        QString f = filter;
        int start, end;
    ...
    void GUIFileDirectoryDialog::onDoubleClickedObject(QTreeWidgetItem* pitem, int column)
    {
    ...
    void GUIFileDirectoryDialog::onFilterChange(QString filter)
    {
        _filterList.clear();
    ...
    void GUIFileDirectoryDialog::filterChange(QString filter)
    {
        _filterList.clear();
        _filterList = GetFilterFromString(filter);
    ...
    void GUIFileDirectoryDialog::GetFileList(QString dirName)
    {
        _fileList.clear();
    ...
    QStringList GUIFileDirectoryDialog::getCurrentFullPath()
    {
        QStringList files;
        //获取结果文件链表
    ...
    bool GUIFileDirectoryDialog::isGourpFiles(const QString path)
    {
        if (path.indexOf("...") > 0)
        {
            return true;
    ...
    void GUIFileDirectoryDialog::iniFileFilterType(QString filterString)
    {
        _ui->FileType->insertItem(0, filterString);
        _ui->FileType->setCurrentIndex(0);
    }

    void GUIFileDirectoryDialog::onCancelClicked()
    {
        close();
    }

    void GUIFileDirectoryDialog::onOkClicked()
    {
    ...
    void GUIFileDirectoryDialog::enterDirectory()
    {
        ...
        for (int i = 0; i < _fileList.count(); i++)
        {
            ...
            fileHeadExt = fileHead + GUIFileDirectoryDialog::splitExt + fileExt;
    ...
    void GUIFileDirectoryDialog::SetDirectoryForDrives()
    {
    ...
    void GUIFileDirectoryDialog::onItemSelectionChanged()
    {
    ...
    void GUIFileDirectoryDialog::func_PathTreeViewDoubleClick(const QModelIndex& index)
    {
        _currentIndex = index;
    ...
    void GUIFileDirectoryDialog::unix_PathTreeViewDoubleClick(QString itemName)
    {
    ...
    void GUIFileDirectoryDialog::pathTreeViewDoubleClick(const QModelIndex& index)
    {
#ifdef Q_OS_WIN
    ...
    QString GUIFileDirectoryDialog::FindFullFilename(const QModelIndex& index)
    {
        if (index.data().toString() == NULL) return NULL;
        return FindFullFilename(index.parent()) + "/" + index.data().toString();
    }

    void GUIFileDirectoryDialog::refreshRightView()
    {
    ...
    void GUIFileDirectoryDialog::onUpPushButtonClicked()
    {
#ifdef Q_OS_WIN
    ...
    void GUIFileDirectoryDialog::onBackClicked()
    {
    ...
    void GUIFileDirectoryDialog::onFrontClicked()
    {
    ...
    QStringList GUIFileDirectoryDialog::getSelectGroupFiles(const QString path)
    {
