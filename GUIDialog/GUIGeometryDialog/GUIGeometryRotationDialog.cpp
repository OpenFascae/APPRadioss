#include "GUIGeometryRotationDialog.h"
#include "ui_GUIGeometryRotationDialog.h"
...
namespace GUI {
    GUIGeometryRotationDialog::GUIGeometryRotationDialog(Core::FITKActionOperator* oper, QWidget* parent)
        :Core::FITKDialog(parent), _oper(oper)
    {
        _ui = new Ui::GUIGeometryRotationDialog();
        _ui->setupUi(this);
        // 设置对话框关闭时自动删除
        ...
        {
            connect(pickProvider, &GraphData::PickedDataProvider::sigDataPicked, this, &GUIGeometryRotationDialog::pickFinishedOper);
        }
        this->init();
    }

    GUIGeometryRotationDialog::~GUIGeometryRotationDialog()
    {
        if (_ui) {
            delete _ui;
            _ui = nullptr;
    ...
    void GUIGeometryRotationDialog::init()
    {
        //获取几何管理器
        ...
        QString name = _geometryData->checkName(tr("Rotation-1"));
        _ui->lineEdit_Name->setText(name);
        setWindowTitle(tr("Create Rotation"));
    }

    void GUIGeometryRotationDialog::pickFinishedOper()
    {
        //获取拾取数据
        GraphData::PickedDataProvider* pickProvider = GraphData::PickedDataProvider::getInstance();
        if (!pickProvider) return;
    ...
    void GUIGeometryRotationDialog::on_pushButton_OK_clicked()
    {
        ...
        Interface::FITKAbsGeoModelRotate*roatetP = dynamic_cast<Interface::FITKAbsGeoModelRotate*>(geoFactory->createCommand(Interface::FITKGeoEnum::FITKGeometryComType::FGTRotate));
        if (roatetP == nullptr) return;
        roatetP->setSourceShape(_virtualOriginalModel);
        //设置轴线起点
        ...
        double angle = _ui->Value->text().toDouble();
        ...
        else
        {
            ...
            roatetP->addReferenceCmdID(_virtualOriginalModel.CmdId);
        }
        int objID = _geometryData->getDataObjectID();
        roatetP->setDataObjectName(name);
        if (_geometryData->getDataObjectName().isEmpty())
        {
            Interface::FITKGeoCommandList* cmdList = FITKAPP->getGlobalData()->getGeometryData<Interface::FITKGeoCommandList>();
            if (cmdList == nullptr) return;
        ...
        if (_geometryData->addAndUpdate(roatetP) == false)
        {
            //创建失败不将该数据append
    ...
    void GUIGeometryRotationDialog::on_pushButton_Cancel_clicked()
    {
        //关闭窗口
    ...
    void GUIGeometryRotationDialog::on_pushButton_Pick_clicked()
    {
        //设置拾取方式
    ...
    void GUIGeometryRotationDialog::on_point_1_pick_clicked()
    {
        //设置拾取方式
        _pickFlag = PickFlag::Point1;
        enablePickSig(GUI::GUIPickInfo::PickObjType::POBGeoVert, GUI::GUIPickInfo::PickMethod::PMSingle);
    }
    void GUIGeometryRotationDialog::on_point_2_pick_clicked()
    {
        //设置拾取方式
        _pickFlag = PickFlag::Point2;
        enablePickSig(GUI::GUIPickInfo::PickObjType::POBGeoVert, GUI::GUIPickInfo::PickMethod::PMSingle);
    }
    void GUIGeometryRotationDialog::enablePickSig(GUI::GUIPickInfo::PickObjType objType, GUI::GUIPickInfo::PickMethod methodType)
    {
    ...
    std::array<double, 3> GUIGeometryRotationDialog::getPointXYZ(int id)
    {
