...
namespace GUI {
    ...
    void GUIGeometryCylinderDialog::on_pushButton_OK_clicked()
    {
        ...
        Interface::FITKAbsGeoModelCylinder* cylinderModel = dynamic_cast<Interface::FITKAbsGeoModelCylinder*>(geoFactory->createCommand(Interface::FITKGeoEnum::FITKGeometryComType::FGTCylinder));
        ...
        cylinderModel->setDataObjectName(name);
        if (_geometryData->getDataObjectName().isEmpty())
        {
            Interface::FITKGeoCommandList* cmdList = FITKAPP->getGlobalData()->getGeometryData<Interface::FITKGeoCommandList>();
            if (cmdList == nullptr) return;
        ...
        if (_geometryData->addAndUpdate(cylinderModel) == false)
        {
            //创建失败不将该数据append
