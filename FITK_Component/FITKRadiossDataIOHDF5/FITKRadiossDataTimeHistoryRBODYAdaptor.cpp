#include "FITKRadiossDataTimeHistoryRBODYAdaptor.h"
...
namespace Interface
{
    bool FITKRadiossDataTimeHistoryRBODYAdaptor::adaptR()
    {
        _data = dynamic_cast<Radioss::FITKTimeHistoryRBODY*>(_dataObj);
        ...
        {
            _data->setVarlableGroupType(static_cast<Radioss::FITKTimeHistoryRBODY::THRBODYVariableGroup>(intValue));
        }
        // 读取对象ID列表
        std::string strValue{};
        ...
        if (h5tools->readGroupStr(*_h5Group, "CustomizeVariable", strValue))
        {
            ...
            QList<Radioss::FITKTimeHistoryRBODY::TimeHistoryRBODYVariable> customizeVars;

            Core::FITKEnumTransfer<Radioss::FITKTimeHistoryRBODY::TimeHistoryRBODYVariable> varTransfer;
            bool isValid = false;

            for (const QString& varStr : customizeVarList)
            {
                Radioss::FITKTimeHistoryRBODY::TimeHistoryRBODYVariable varType =
    ...
    bool FITKRadiossDataTimeHistoryRBODYAdaptor::adaptW()
    {
        _data = dynamic_cast<Radioss::FITKTimeHistoryRBODY*>(_dataObj);
        ...
        // 写入自定义变量
        QList<Radioss::FITKTimeHistoryRBODY::TimeHistoryRBODYVariable> customizeVars = _data->getCustomizeVariable();
        QStringList customizeVarStrs{};
        Core::FITKEnumTransfer<Radioss::FITKTimeHistoryRBODY::TimeHistoryRBODYVariable> varTransfer;
        bool isValid = false;
        for (const Radioss::FITKTimeHistoryRBODY::TimeHistoryRBODYVariable& var : customizeVars)
        {
        ...
        QString type = Radioss::FITKTimeHistoryRBODY::GetFITKTimeHistoryRBODYRadiossKeyWord();
