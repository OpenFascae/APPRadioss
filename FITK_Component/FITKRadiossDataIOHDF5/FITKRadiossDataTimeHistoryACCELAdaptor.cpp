...
#include "FITKRadiossDataTimeHistoryACCELAdaptor.h"
...
namespace Interface
{
    bool FITKRadiossDataTimeHistoryACCELAdaptor::adaptR()
    {
        _data = dynamic_cast<Radioss::FITKTimeHistoryACCEL*>(_dataObj);
        ...
        {
            _data->setVarlableGroupType(static_cast<Radioss::FITKTimeHistoryACCEL::THACCELVariableGroup>(intValue));
        }
        // 读取对象ID列表
        std::string strValue{};
        ...
        if (h5tools->readGroupStr(*_h5Group, "CustomizeVariable", strValue))
        {
            ...
            QList<Radioss::FITKTimeHistoryACCEL::TimeHistoryACCELVariable> customizeVars;

            Core::FITKEnumTransfer<Radioss::FITKTimeHistoryACCEL::TimeHistoryACCELVariable> varTransfer;
            bool isValid = false;

            for (const QString& varStr : customizeVarList)
            {
                Radioss::FITKTimeHistoryACCEL::TimeHistoryACCELVariable varType =
    ...
    bool FITKRadiossDataTimeHistoryACCELAdaptor::adaptW()
    {
        _data = dynamic_cast<Radioss::FITKTimeHistoryACCEL*>(_dataObj);
        ...
        // 写入自定义变量
        QList<Radioss::FITKTimeHistoryACCEL::TimeHistoryACCELVariable> customizeVars = _data->getCustomizeVariable();
        QStringList customizeVarStrs{};
        Core::FITKEnumTransfer<Radioss::FITKTimeHistoryACCEL::TimeHistoryACCELVariable> varTransfer;
        bool isValid = false;
        for (const Radioss::FITKTimeHistoryACCEL::TimeHistoryACCELVariable& var : customizeVars)
        {
        ...
        QString type = Radioss::FITKTimeHistoryACCEL::GetFITKTimeHistoryACCELRadiossKeyWord();
