...
#include "FITKRadiossDataTimeHistorySECTIOAdaptor.h"
...
namespace Interface
{
    bool FITKRadiossDataTimeHistorySECTIOAdaptor::adaptR()
    {
        _data = dynamic_cast<Radioss::FITKTimeHistorySECTIO*>(_dataObj);
        ...
        {
            _data->setVarlableGroupType(static_cast<Radioss::FITKTimeHistorySECTIO::THSECTIOVariableGroup>(intValue));
        }
        // 读取对象ID列表
        std::string strValue{};
        ...
        if (h5tools->readGroupStr(*_h5Group, "CustomizeVariable", strValue))
        {
            ...
            QList<Radioss::FITKTimeHistorySECTIO::TimeHistorySECTIOVariable> customizeVars;

            Core::FITKEnumTransfer<Radioss::FITKTimeHistorySECTIO::TimeHistorySECTIOVariable> varTransfer;
            bool isValid = false;

            for (const QString& varStr : customizeVarList)
            {
                Radioss::FITKTimeHistorySECTIO::TimeHistorySECTIOVariable varType =
    ...
    bool FITKRadiossDataTimeHistorySECTIOAdaptor::adaptW()
    {
        _data = dynamic_cast<Radioss::FITKTimeHistorySECTIO*>(_dataObj);
        ...
        // 写入自定义变量
        QList<Radioss::FITKTimeHistorySECTIO::TimeHistorySECTIOVariable> customizeVars = _data->getCustomizeVariable();
        QStringList customizeVarStrs{};
        Core::FITKEnumTransfer<Radioss::FITKTimeHistorySECTIO::TimeHistorySECTIOVariable> varTransfer;
        bool isValid = false;
        for (const Radioss::FITKTimeHistorySECTIO::TimeHistorySECTIOVariable& var : customizeVars)
        {
        ...
        QString type = Radioss::FITKTimeHistorySECTIO::GetFITKTimeHistorySECTIORadiossKeyWord();
