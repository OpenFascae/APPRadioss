#include "FITKRadiossAdaptorBCSLagmul.h"
...
namespace Radioss
{
    QString FITKRadiossAdaptorBCSLagMul::getAdaptorClass()
    {
        return "Radioss::FITKRadiossAdaptorBCSLagMul";
    }

    bool FITKRadiossAdaptorBCSLagMul::adaptR()
    {
        if (_caseData == nullptr || _reader == nullptr) return false;
        
        // 创建边界条件数据对象
        FITKRadiossBCSLagMul* bcs = new FITKRadiossBCSLagMul();
        //读取关键字
    ...
    bool FITKRadiossAdaptorBCSLagMul::adaptW()
    {
        FITKRadiossBCSLagMul* bc = dynamic_cast<FITKRadiossBCSLagMul*>(_dataObj);
        ...
        *stream << bc->GetFITKRadiossBCSLagMulRadiossKeyWord() << "/" << radID << endl;
        //写出名称
        *stream << bc->getDataObjectName() << endl;
        //节点组ID
        int grnd = infoMapper->getRadIDByNodeSetID(bc->getGroupID1());
        //自由度
        QString DOF = QString("   %1%2%3 %4%5%6").arg(bc->isFreedomFixed(0)).arg(bc->isFreedomFixed(1)).arg(bc->isFreedomFixed(2))
            .arg(bc->isFreedomFixed(3)).arg(bc->isFreedomFixed(4)).arg(bc->isFreedomFixed(5));
    ...
    bool FITKRadiossAdaptorBCSLagMul::readBCS(FITKRadiossBCSLagMul * bcs)
    {
    ...
    bool FITKRadiossAdaptorBCSLagMul::readParam(const QStringList& lines, FITKRadiossBCSLagMul * bcs)
    {
